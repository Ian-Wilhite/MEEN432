%% p4_week2_tune.m
% Optional helper script for Week 2.
% Sweeps target speeds and reports the fastest VALID run.

clc; clear; clf;
p4_init;

candidate_mph = carData.week2_speed_candidates_mph;
summary = nan(numel(candidate_mph), 6);

fprintf('Sweeping candidate target speeds for Week 2...\n\n');
...
for k = 1:numel(candidate_mph)
    ...
    results = evaluate_week2_run_local(X(:), Y(:), vx(:), soc(:), path);

    summary(k,:) = [mph, results.laps, min(soc)*100, max(soc)*100, results.max_cte, results.week2_valid];

    fprintf('  laps = %.2f | min SOC = %.2f%% | max CTE = %.2f m | valid = %d\n\n', ...
        results.laps, min(soc)*100, results.max_cte, results.week2_valid);
end
...
if isempty(valid_idx)
    fprintf('No valid candidate found. Reduce speed further or decrease lookahead more.\n');
else
...
function results = evaluate_week2_run_local(X, Y, vx, soc, path)
    cx = path.xpath(:);
    cy = path.ypath(:);
    nv = length(X);
