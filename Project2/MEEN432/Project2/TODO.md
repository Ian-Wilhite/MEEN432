...
- [ ] Export track waypoints (`path` struct) from MATLAB into Simulink (Data Store, From Workspace, or Bus Creator) so the model uses the same track definition as the animation script.
- [ ] Integrate `raceStat` scoring after the Simulink run (can keep the helper inside `animate.m` or move to its own file) and fix the formatting of the loop print statement.
- [ ] Verify the animated patch rotates correctly with heading and that car dimensions/offsets make sense relative to track width; consider leaving motion trail for visualization.
- [ ] Add error handling/guardrails in scripts (`run.m`, `animate.m`) for missing simulation data and empty Simulink runs.
- [ ] Document run instructions and assumptions in `README.md` (MATLAB/Simulink version, required toolboxes, how to change speeds/params) and include final performance notes when available.
