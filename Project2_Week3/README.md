...
This project implements a closed-loop vehicle simulation in Simulink that drives a car around a generated oval race track using the Week 2 lateral/kinematic model. The workflow is: (1) `init.m` defines vehicle parameters (mass, inertia, tire cornering stiffness, wheelbase, limits), initial conditions, and creates the Simulink bus; (2) `gentrack.m` generates the track centerline and inner/outer boundaries and exports both `path` (used by the Simulink model) and `track` (required field names for statistics); (3) the Simulink model computes steering using a driver/controller and propagates the vehicle states (X, Y, heading ψ); and (4) `animate.m` runs the model, animates the car motion on the track, and calls the provided `raceStat.m` script to compute number of loops, lap time, total distance, average speed, and whether the vehicle leaves the track. To meet the “To Workspace” requirement, the model should include To Workspace blocks for `X`, `Y`, and `psi` (or `car_X`, `car_Y`, `car_psi`); `animate.m` supports either To Workspace variables or logged outputs returned by `sim()`. With the current PD + feedforward steering approach and a constant target speed of 15 m/s, the vehicle tracks the oval well and (in our tuning) remains inside the boundaries for the full lap. When the car goes out of bounds, the usual causes are controller gains that are too aggressive (overshoot in curves), insufficient lookahead/preview, or lateral force saturation causing understeer; reducing speed, increasing lookahead, or retuning gains resolves this.
...
### Checking outputs
- After a run, the workspace contains `path`, `track`, and `race` (statistics struct).
- If you are using To Workspace blocks, confirm that `X`, `Y`, and `psi` exist in the workspace after simulation.
